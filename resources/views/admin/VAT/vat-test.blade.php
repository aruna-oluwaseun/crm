...
{
    $clientId = env('TEST_HMRC_CLIENT');
    $clientSecret = env('TEST_HMRC_SECRET');
    $accessToken = $franchise->hmrc_test_access_token;
}
else
{
    $clientId = env('HMRC_CLIENT');
    $clientSecret = env('HMRC_SECRET');
    $accessToken = $franchise->hmrc_access_token;
}

$accessToken = false;
$serverToken = 'df3bd5adc443307a24d7e496f579d420';

refreshAccessTokenIfNeeded($clientId, $clientSecret, $accessToken);

$vrn = '178636519';
$vatObligationFrom = '2021-01-01';
$vatObligationTo = '2022-01-01';
$vatObligationStatus = '';
...
function refreshAccessTokenIfNeeded($client_id, $client_secret, $access_token)
{
    if (!isset($client_id)) {
        return;
    }
    ...
    if(!$existingAccessToken = AccessToken::get())
    {
...
<div style="margin-top: 10px">
    ...
    <a href="javascript:void(0)" onclick="authorize()" class="btn btn-sm btn-warning" style="margin-top: 10px" id="create-access-token-btn">Create access token</a>
</div>
<hr>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover table-sm">
                ...
                <tr>
                    ...
                    <td class="test-btn">
                        <a href='{{ url('vat/hello-world-user') }}'>
                            <button class="btn btn-sm btn-primary">Test</button>
                        </a>
                    </td>
                </tr>
...
<script>
    ...
    function authorize() {
        ...
        if (query.length) {
            location.href = 'vat/create-access-token' + '?' + queryString;
        } else {
            location.href = 'vat/create-access-token';
        }
    }
    ...
    function helloWorldApplication() {
        ...
        const queryString = query.join('&');
        if (query.length) {
            location.href = '/vat/hello-world-application' + '?' + queryString;
        } else {
            location.href = '/vat/hello-world-application';
        }
    }
    ...
    function retrieveVATObligations() {
        ...
        const queryString = query.join('&');
        if (query.length) {
            location.href = "/examples/vat/get-vat-obligations" + '?' + queryString;
        } else {
            location.href = "/examples/vat/get-vat-obligations";
        }
    }
    ...
    function submitVATReturn() {
        ...
        const queryString = query.join('&');
        if (query.length) {
            location.href = "/examples/vat/submit-vat-return" + '?' + queryString;
        } else {
            location.href = "/examples/vat/submit-vat-return";
        }
    }
    ...
    function viewVATReturn() {
        ...
        const queryString = query.join('&');
        if (query.length) {
            location.href = "/examples/vat/view-vat-return" + '?' + queryString;
        } else {
            location.href = "/examples/vat/view-vat-return";
        }
    }
</script>
</body>
