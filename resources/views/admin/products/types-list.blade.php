                                            ...
                                            <div class="toggle-wrap nk-block-tools-toggle">
                                                ...
                                                <div class="toggle-expand-content" data-content="pageMenu">
                                                    <ul class="nk-block-tools g-3">
                                                        ...
                                                        <li><a href="#" data-toggle="modal" data-target="#modalCreateType" class="btn btn-white btn-primary"><em class="icon ni ni-plus"></em><span>Add Product Type</span></a></li>
                                                        @endcan
                                                ...
                                                <?php if(isset($types) && $types->count()) : ?>
                                                    ...
                                                    <?php foreach($types as $item) : ?>
                                                        ...
                                                        <tr class="nk-tb-item data-container" data-id="{{$item->id}}">
                                                            ...
                                                            <td class="nk-tb-col"><a href="{{ url('product-types/'.$item->id) }}">{{ $item->id }}</a></td>
                                                            <td class="nk-tb-col">
                                                                <div class="user-card">
                                                                    <div class="user-info">
                                                                        <span class="tb-lead"><a href="{{ url('product-types/'.$item->id) }}">{{ $item->title }}</a> <!--<span class="dot dot-success d-md-none ml-1"></span>--></span>
                                                                        <span>{{ $item->code }}</span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                                        ...
                                                                        <div class="drodown">
                                                                            ...
                                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                                <ul class="link-list-opt no-bdr">
                                                                                    ...
                                                                                    <li><a href="{{ url('product-types/'.$item->id) }}"><em class="icon ni ni-eye"></em><span>View</span></a></li>
...
<div class="modal fade" tabindex="-1" id="modalCreateType">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ url('product-types') }}" id="create-form" class=" form-validate">
                    @csrf
                    <div class="alert alert-warning" id="fetch-product-status" style="display: none"></div>

                    <div class="form-group">
                        <label class="form-label" for="notes">Product type name *</label>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // slug
        $(document).on('keyup','#modalCreateType [name="title"]', function(event) {
            var title = $(this).val();
            setTimeout(function() {
                var slug = title.toLowerCase().replace(/ /g,'-').replace(/[-]+/g, '-').replace(/[^\w-]+/g,'');
                $('#modalCreateType [name="slug"]').val(slug);
            }, 400);
        });
    });
</script>
