...
$(document).ready(function () {
      ...
      success: function success(response) {
        if (response.success) {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: response.message ? response.message : 'We could not retrieve the items you wish to dispatch.' //footer: '<a href>Why do I have this issue?</a>'
  ...
  $(document).on('change focusout', '#add-additional-shipping-cost, #add-additional-shipping-vat', function (event) {
    var vat_value = $('#add-additional-shipping-vat').find(':selected').data('value'); // Get the inputed item cost

    var item_cost = parseFloat($('#add-additional-shipping-cost').val() ? $('#add-additional-shipping-cost').val() : 0);
    var vat = item_cost * vat_value / 100; //$('#add-additional-shipping-vat-cost').val(vat.toFixed(2));
  ...
  $(document).on('change focusout', '#add-item-cost, #add-vat-type-id, #add-qty', function (event) {
    ...
    var qty = $('#add-qty').val(); // Per item costings

    var original_net_cost = parseFloat($('#add-original-net-cost').val()) * qty;
    var original_sale_cost = parseFloat($('#add-original-sale-cost').val()) * qty; // Get the inputed item cost
      ...
      success: function success(response) {
        if (response.success) {
          ...
          if (response.data.deposit_allowed) {
            var deposit_checkbox = '<div class="custom-control custom-checkbox">' + '<input data-original-net="' + response.data.net_cost + '" data-deposit-net="' + response.data.deposit_net_cost + '" data-deposit-gross="' + response.data.deposit_gross_cost + '" data-deposit-vat="' + response.data.deposit_vat_cost + '" name="paying_deposit" type="checkbox" class="custom-control-input" id="add-paying-deposit" value="1">' + '<label class="custom-control-label" for="add-paying-deposit"> Paying deposit?</label></div>';
            ...
            $.ajax({
              ...
              error: function error() {
                Swal.fire({
                  icon: 'error',
                  title: 'Not found',
                  text: 'Failed to find training data.' //footer: '<a href>Why do I have this issue?</a>'
...
/***/ 5:
/*!*************************************************!*\
  !*** multi ./resources/js/admin/sales-order.js ***!
  \*************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Users/richardholmes/Sites/localhost/jenflow-laravel/resources/js/admin/sales-order.js */"./resources/js/admin/sales-order.js");
